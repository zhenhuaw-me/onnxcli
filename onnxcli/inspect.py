...
class InspectCmd(SubCmd):
    ...
    def add_args(self, subparser):
        ...
        subparser.add_argument('-m', '--meta', action='store_true', help="Print the meta information of the model")
        subparser.add_argument('-n', '--node', action='store_true', help="Print the node information of the model")
        subparser.add_argument('-t', '--tensor', action='store_true', help="Print the tensor information of the model")
        subparser.add_argument('-i', '--index', type=int, default=None, help="Specify the index of the node or tensor to inspect")
    ...
    def print_tensor(self, g, idx=None):
        ...
        if idx is not None:
            if idx >= len(g.value_info) and idx >= len(g.initializer):
                raise ValueError("Index {} out of range, value_info in total {}, initializer in total {}".format(idx, len(g.value_info), len(g.initializer)))
    ...
    def print_node(self, g, idx=None):
        ...
        if idx is not None:
            if idx >= len(g.node):
                raise ValueError("Index {} out of range, nodes in total {}".format(idx, len(g.node)))
